...
8. **Access the application:**
    ...
    http://127.0.0.1:8000
    ```
9. **Run in seprate Terminals**
    ```
    celery -A app.core.utils.celery_worker worker --loglevel=info --pool=threads --concurrency=1 --queues=analysis_session_queue
    ```
    ```
    celery -A app.core.utils.celery_worker worker --loglevel=info --pool=threads --concurrency=1 --queues=validation_session_queue
    ```
10. **Running tests**
