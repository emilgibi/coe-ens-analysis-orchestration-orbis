...
class UploadSupplierMasterData(Base):
    ...
    client_onboarding_date = Column(Date, nullable=True)
    ...
    validation_status = Column(SQLAlchemyEnum(ValidationStatus), nullable=False,
                               server_default=expression.literal(ValidationStatus.PENDING.value))
    final_status = Column(SQLAlchemyEnum(FinalStatus), nullable=False,
                          server_default=expression.literal(FinalStatus.PENDING.value))
    # new
    final_validation_status = Column(SQLAlchemyEnum(FinalValidatedStatus), nullable=False,
                                     server_default=expression.literal(FinalValidatedStatus.PENDING.value))
    orbis_matched_status = Column(SQLAlchemyEnum(OribisMatchStatus), nullable=False,
                                  server_default=expression.literal(OribisMatchStatus.PENDING.value))

    truesight_status = Column(SQLAlchemyEnum(TruesightStatus), nullable=False,
                              server_default=expression.literal(TruesightStatus.PENDING.value))
    ...
    process_status = Column(SQLAlchemyEnum(STATUS), nullable=False,
                            server_default=expression.literal(STATUS.PENDING.value))
    user_id = Column(String, ForeignKey("users_table.user_id", ondelete="CASCADE"))  # Change to correct PK
    uploaded_external_vendor_id = Column(String, nullable=True)
    duplicate_in_session = Column(SQLAlchemyEnum(DUPINSESSION), nullable=False,
                                  server_default=expression.literal(DUPINSESSION.UNIQUE.value))
    existing_entity = Column(SQLAlchemyEnum(ExistingType), nullable=True)
...
class ExternalSupplierData(Base):
    ...
    cyber_data  = Column(JSONB, nullable=True)
...
class GridManagement(Base):
    ...
    grid_legal = Column(JSONB, nullable=True)
    management_info = Column(JSONB, nullable=True)
...
class WebhookResponse(Base):
    ...
    ens_id = Column(String(50))
    session_id = Column(String(50))
    response_id = Column(String(100))
...
class GoogleRating(Base):
    ...
    bvd_id = Column(String(100), nullable=True)
    create_time = Column(DateTime(timezone=True), server_default=func.now())
...
class KpiEnt(KPISchemas):
    __tablename__ = "entity_existence"
    __table_args__ = (
        UniqueConstraint('ens_id', 'session_id', 'kpi_code', name='unique_ensid_session_kpient'),
    )