...
async def financial_ratios_analysis(data, session):
    ...
    try:
        ...
        if quick_ratio_value:
            ...
            BKR4A["kpi_data"] = [f"Quick Ratio: {BKR4A['kpi_rating']} ({quick_ratio_value})"]
        else:
            BKR4A["kpi_rating"] = "INFO"
            BKR4A["kpi_details"] = "No quick ratio information available"
            BKR4A["kpi_data"] = ["No quick ratio information available"]
        ...
        if current_ratio not in [None, "n.a"]:
            try:
                ...
                BKR5A["kpi_value"] = str(round(current_ratio_float, 2))
                BKR5A["kpi_flag"] = True
                BKR5A["kpi_details"] = f"Current Ratio: {BKR5A['kpi_rating']} ({round(current_ratio_float, 2)})"
                BKR5A["kpi_data"] = [f"Current Ratio: {BKR5A['kpi_rating']} ({round(current_ratio_float, 2)})"]
            except (ValueError, TypeError) as e:
                BKR5A["kpi_rating"] = "INFO"
                BKR5A["kpi_details"] = f"Invalid current ratio value: {str(e)}"
                BKR5A["kpi_data"] = [f"Invalid current ratio value: {str(e)}"]
        else:
            BKR5A["kpi_rating"] = "INFO"
            BKR5A["kpi_details"] = "No current ratio information available"
            BKR5A["kpi_data"] = ["No current ratio information available"]
        ...
        if all(x not in [None, "n.a"] for x in [short_debt, long_debt, total_equity]) and total_equity != 0:
            try:
                ...
                if total_equity_float == 0:
                    ...
                    BKR6A["kpi_data"] = ["Debt to Equity Ratio: High (Total equity is zero)"]
                else:
                    ...
                    BKR6A["kpi_data"] = [f"Debt to Equity Ratio: {BKR6A['kpi_rating']} ({round(debt_to_equity, 2)})"]
            except (ValueError, TypeError):
                BKR6A["kpi_rating"] = "INFO"
                BKR6A["kpi_details"] = "Invalid debt or equity values"
                BKR6A["kpi_data"] = ["Invalid debt or equity values"]
        else:
            BKR6A["kpi_rating"] = "INFO"
            BKR6A["kpi_details"] = "No debt to equity information available"
            BKR6A["kpi_data"] = ["No debt to equity information available"]
    ...
    try:
        ...
        for idx, col in enumerate(required_columns, start=1):
            ...
            if metric_data and len(metric_data) > 0:
                has_valid_data = any(
                    val.get("value") is not None and
                    val.get("value") != "" and
                    val.get("value") != "None" and
                    str(val.get("value")).strip() != "" and
                    val.get("closing_date") is not None and
                    str(val.get("closing_date")).strip() != ""
                    for val in metric_data if isinstance(val, dict)
                )
                ...
                for val in metric_data:
                    if (val.get("value") is not None and
                            val.get("value") != "" and
                            val.get("value") != "None" and
                            str(val.get("value")).strip() != "" and
                            val.get("closing_date") is not None and
                        ...
                        if col.lower() in ratios:
                            try:
                                ratio_val = round(float(val.get("value")), 2)
                            except (ValueError, TypeError):
                                ratio_val = val.get("value")
                ...
                if formatted_metric_data:
                    ...
                    kpi_entry["kpi_data"] = [f"[{val['closing_date']}]: {val['value']}" for val in formatted_metric_data]
