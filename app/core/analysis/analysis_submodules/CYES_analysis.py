...
async def esg_analysis(data, session):
    ...
    try:
        ...
        # await data_fetching_for_new_and_old_session_additional_indicator(ens_id_value, session_id_value,required_columns)
...
async def cyber_analysis(data, session):
    ...
    try:
        ...
        # await data_fetching_for_new_and_old_session_additional_indicator(ens_id_value,session_id_value,required_columns[:4])
        retrieved_data = await get_dynamic_ens_data("external_supplier_data", required_columns, ens_id_value, session_id_value, session)
        retrieved_data = retrieved_data[0]
        ...
        if implied_cyber_risk is not None:
            if "very high" in implied_cyber_risk.lower():
                ...
                CYB1B["kpi_data"] = [f"High Implied Cyber Risk"]
        ...
        if CYB1A["kpi_rating"] == "High":
            ...
            if CYB1B["kpi_rating"] == "High" or CYB1B["kpi_rating"] == "Medium":
                ...
                CYB2A["kpi_data"] = [f"Final Cyber Risk Rating: Low"]
        else:
...
async def cyber_analysis_api(data, session):
    ...
    try:
        ...
        CYB3A = kpi_template.copy()
        CYB3A["kpi_code"] = "CYB1A"
        CYB3A["kpi_definition"] = "Cyber Risk Validation"

        required_columns = ["cyber_data"]
        ...
        retrieved_data = retrieved_data[0]
        cyber_info = retrieved_data.get("cyber_data")
        ...
        if cyber_info.get("cyber_risk_level", "N/A"):
            ...
            CYB3A["kpi_flag"] = True
            print("check 2")
            CYB3A["kpi_rating"] = cyber_info.get("cyber_risk_level", "N/A")
            CYB3A["kpi_value"] = json.dumps(result)
            CYB3A["kpi_details"] = json.dumps(result)
            print("check 3")
