    ...
    try:
        ...
        if len(onf) > 0:
            ...
            for event in onf:
                ...
                current_unique_detail = f"{event.get('TITLE')}: {event.get('TOPIC')} - {truncate_string(event.get('ARTICLE'))} \n Source:{event.get('SOURCE', 'N/A')} | Publication: {event.get('PUBLICATION', 'N/A')} | (Date: {event.get('DATE')[:10]})"
                unique_data_points.append(current_unique_detail)
            ...
            data_point_counter = 1
            details = "Other News Findings are as follows:\n"
            for finding in unique_data_points:
                finding = f"{data_point_counter}. {finding}\n"
                details += finding
                data_point_counter += 1
            ...
            ONF1A["kpi_details"] = details
            ONF1A["kpi_data"] = unique_data_points
