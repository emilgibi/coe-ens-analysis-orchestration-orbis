...
############################################################################################################################
        ...
        if upload_to_blob:
            ...
            try:
                ...
                docx_upload_success = upload_to_r2(docx_buffer, docx_file, session_id)
            ...
            try:
                ...
                status = upload_to_r2(pdf_buffer_, pdf_file, session_id)
...
async def report_generation_poc(data, session, diff_notification, annexure_from_diff, upload_to_blob: bool, save_locally: bool, ts_data=None):
    ...
    try:
        ...
        # Sanctions DataFrames
        try:
            data = await populate_entity(incoming_ens_id=incoming_ens_id, incoming_session_id=session_id,
                                            session=session)
            temp = data["entity"]
            if not temp.empty:
                sape_data = temp.to_dict(orient='records')
                ...
                context["entity_existence_data"] = sape_data
            else:
        ...
        try:
            ...
            if not bribery_df.empty:
                temp = anti_corruption_data["bribery"]
                bribery_data = temp.to_dict(orient='records')
                logger.debug("bribery data 1",bribery_data)
                bribery_data = remove_time_keys(bribery_data)

                #for testing
                # bribery_data[0]['kpi_details']+="Theft (Larceny, Misappropriation, Embezzlement, Extortion): Associated with, Seen with - Dong-A Ilbo listed an article on May 30, 2024, re: The case against the president of the Planning and Coordination Office of Hyundai Motor Group and the former president of Hyundai Engineering & Construction, who were suspected of embezzlement and [...] (Date: 2024-05-30 00:00:00)\n "
                context["bribery_findings"] = True
                for n in bribery_data:
                    kpi_code=n['kpi_code']
                    if kpi_code in all_diff_code:
                        logger.debug(f"bcf present for code:{kpi_code}")
                        logger.debug(diff_notification[kpi_code] or 'xyz')
                        details=n['kpi_details']
                        for sentence in diff_notification[kpi_code]:
                            if sentence in details:
                                logger.debug("sentence found")
                                details=details.replace(sentence,f'<p>{sentence}</p>')
                        n['kpi_details']=details
