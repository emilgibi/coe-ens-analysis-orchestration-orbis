...
async def supplier_name_validation(data, session, search_engine:str):
    ...
    try:
        ...
        if not matched and not potential_pass:
            ...
            # Check if ens_id exists in entity_universe and assign indicator
            universe_row = await get_dynamic_ens_data("entity_universe", ["ens_id"], incoming_ens_id, None, session)
            if universe_row:
                updated_data["existing_entity"] = ExistingType.EXISTING.value
            else:
                updated_data["existing_entity"] = ExistingType.NEW.value
